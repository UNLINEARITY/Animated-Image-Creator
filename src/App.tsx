          ...
          <div className="controls-bar">
            ...
            <div className="control-group">
              ...
              <input
                type="text"
                className="frame-delay-input"
                style={{width: '80px', padding: '0.4rem', textAlign: 'center', fontSize: loopCount === 0 ? '1.4rem' : undefined, lineHeight: loopCount === 0 ? 1.1 : undefined}}
                value={loopCount === 0 ? '∞' : String(loopCount)}
                disabled={loopCount === 0}
                title={loopCount === 0 ? 'Infinite loop' : 'Number of plays (1-65535)'}
                onChange={(e) => {
                  const v = parseInt(e.target.value.replace(/\D/g, ''), 10);
                  // WebP loop field is 16-bit; 0 is reserved for infinite in both formats
                  setLoopCount(Number.isFinite(v) ? Math.min(Math.max(v, 1), 65535) : 1);
                }}
              />
              <label
                style={{display: 'flex', alignItems: 'center', gap: '0.35rem', cursor: 'pointer', userSelect: 'none'}}
                title="Loop the animation forever"
              >
                <input
                  type="checkbox"
                  checked={loopCount === 0}
